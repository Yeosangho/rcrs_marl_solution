...
public class SamplePoliceForce extends AbstractSampleAgent<PoliceForce> {
  ...
  private static final Logger LOG          = Logger
      .getLogger( SamplePoliceForce.class );
   ...
   * for (EntityID blockadeID : area.getBlockades()) { Logger.debug("Checking "
   * + blockadeID); StandardEntity entity = model.getEntity(blockadeID);
   * Logger.debug("Found " + entity); if (entity == null) { continue; }
   * Pair<Integer, Integer> location = entity.getLocation(model);
   * Logger.debug("Location: " + location); if (location == null) { continue; }
   * double dx = location.first() - x; double dy = location.second() - y; double
   * distance = Math.hypot(dx, dy); if (best == null || distance < bestDistance)
   * { bestDistance = distance; best = entity.getID(); } } }
   * Logger.debug("Nearest blockade: " + best); return best; }
   */
}
