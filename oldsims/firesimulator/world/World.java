...
public class World implements WorldConstants {
    ...
    public World() {
        ...
        buildings = new LinkedHashSet<Building>();
    ...
    private void loadVars() {
        ...
        FireBrigade.REFILL_HYDRANT_QUANTITY = new Integer(Configuration.getValue("resq-fire.water_hydrant_refill_rate"))
                .intValue();
    ...
    /*
     * private void initializeRoads() { for(Iterator
     * i=streetNodes.iterator();i.hasNext();){ StreetNode sn=(StreetNode)i.next();
     * if(sn.getX()>maxX)maxX=sn.getX(); if(sn.getX()<minX)minX=sn.getX();
     * if(sn.getY()>maxY)maxY=sn.getY(); if(sn.getY()<minY)minY=sn.getY(); }
     * for(Iterator
     * i=roads.iterator();i.hasNext();((Road)i.next()).initialize(this)); }
     */
    ...
    private void initializeAir() {
        ...
        LOG.info("grid cell size=" + SAMPLE_SIZE + "mm, x*y=" + xSamples + "*" + ySamples + " = "
                + (xSamples * ySamples));
    ...
    /*
     * public void processChangeSet(InputBuffer data, int time){ setTime(time); int
     * count = data.readInt(); for (int i = 0; i < count; ++i) { int id =
     * data.readInt(); int propCount = data.readInt(); RescueObject obj =
     * getObject(id); if (obj == null) { LOG.warn("Unknown object (id:"+id+")"); }
     * for (int j = 0; j < propCount; ++j) { String propUrn = data.readString();
     * setProperty(data, obj, propUrn); } } }
     * 
     * private void setProperty(InputBuffer data, RescueObject obj, String urn) {
     * int size = data.readInt(); int[] val; if (urn.equals("SIGNAL_TIMING") ||
     * urn.equals("POSITION_HISTORY") || urn.equals("EDGES") ||
     * urn.equals("SHORTCUT_TO_TURN") || urn.equals("ENTRANCES") ||
     * urn.equals("BUILDING_APEXES") || urn.equals("POCKET_TO_TURN_ACROSS")) { val =
     * new int[data.readInt()]; for (int i = 0; i < val.length; ++i) { val[i] =
     * data.readInt(); } } else { val = new int[] {data.readInt()}; } if (obj !=
     * null) { obj.input(urn, val); } }
     * 
     * private RescueObject createObject(String urn, int id){ RescueObject obj=null;
     * if (urn.equals("BUILDING")) { obj=new Building(id);
     * buildings.add((Building)obj); } else if (urn.equals("REFUGE")) { obj=new
     * Refuge(id); buildings.add((Building)obj); } else if (urn.equals("WORLD")) {
     * obj=new WorldInfo(id); } // else if (urn.equals("ROAD")) { // obj=new
     * Road(id); // roads.add(obj); // } // else if (urn.equals("NODE")) { //
     * obj=new StreetNode(id); // streetNodes.add(obj); // } else if
     * (urn.equals("CIVILIAN")) { obj=new Civilian(id); } else if
     * (urn.equals("CAR")) { obj=new Civilian(id); } else if
     * (urn.equals("AMBULANCE_TEAM")) { obj=new AmbulanceTeam(id); } else if
     * (urn.equals("FIRE_BRIGADE")) { obj=new FireBrigade(id);
     * firebrigades.add(obj); } else if (urn.equals("POLICE_FORCE")) { obj=new
     * PoliceForce(id); } else if (urn.equals("AMBULANCE_CENTRE")) { obj=new
     * AmbulanceCenter(id); buildings.add((Building)obj); } else if
     * (urn.equals("FIRE_STATION")) { obj=new FireStation(id);
     * buildings.add((Building)obj); } else if (urn.equals("POLICE_OFFICE")) {
     * obj=new PoliceOffice(id); buildings.add((Building)obj); } // Moving objects
     * need the world to get their position if (obj instanceof MovingObject)
     * ((MovingObject) obj).setWorld(this); return obj; }
     */
    ...
    public void reset(int episodeNum) {
        ...
        setTime(0);
        resetAir();
        for (Iterator i = buildings.iterator(); i.hasNext(); ((Building) i.next()).reset(this, episodeNum))
            ;
     ...
     * LOG.info("processing commands..."); data.readInt(); // Skip the size
     * data.readInt(); // Skip the simulator ID int time = data.readInt();
     * setTime(time); int count = data.readInt(); for (int i = 0; i < count; ++i) {
     * String cmd = data.readString(); int size = data.readInt(); if
     * ("AK_EXTINGUISH".equals(cmd)) { LOG.debug("EXTINGUISH"); int agentID =
     * data.readInt(); data.readInt(); // Skip the command time
     * LOG.debug("fb.id="+agentID); FireBrigade
     * source=(FireBrigade)getObject(agentID); source.setCurrentAction(cmd); int
     * targetID = data.readInt(); Building target=(Building)getObject(targetID); int
     * quantity=data.readInt(); ExtinguishRequest er=new
     * ExtinguishRequest(source,target,quantity); extinguishRequests.add(er); } else
     * if ("AK_MOVE".equals(cmd)) { int agentID = data.readInt();
     * LOG.debug("MOVE (id="+agentID+")"); MovingObject obj =
     * (MovingObject)getObject(agentID); obj.setCurrentAction(cmd); data.skip(size -
     * 4); } else { LOG.debug("Ignoring " + cmd); LOG.debug("Skipping " + size +
     * " bytes"); data.skip(size); } } }
     */
