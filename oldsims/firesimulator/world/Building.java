...
public class Building extends StationaryObject {
    ...
    public double radiationImpact = 0;
    ...
    public Building(int id, Building b) {
        super(id);
        this.burnRate = b.burnRate;
        this.fierynessChanged = b.fierynessChanged;
        this.waterQuantity = b.waterQuantity;
        this.floors = b.floors;
        this.ignition = b.ignition;
        this.fieryness = b.fieryness;
        this.brokenness = b.brokenness;
        this.cells = new int[b.cells.length][b.cells[0].length];
        for (int i = 0; i < b.cells.length; i++) {
            for (int j = 0; j < b.cells[i].length; j++) {
                this.cells[i][j] = b.cells[i][j];
            }
        }
        // this.entrances = new int[b.entrances.length];
        // for(int i=0; i<b.entrances.length; i++){
        // this.entrances[i] = b.entrances[i];
        // }
        this.code = b.code;
        this.buildingAreaGround = b.buildingAreaGround;
        this.buildingAreaTotal = b.buildingAreaTotal;
        this.apexes = b.apexes;
        this.polygon = b.polygon;
        this.fuel = b.fuel;
        this.initFuel = b.initFuel;
        this.prevBurned = b.prevBurned;
        this.volume = b.volume;
        this.capacity = b.capacity;
        this.energy = b.energy;
        this.cooling = b.cooling;
        this.walls = b.walls;
        this.connectedBuildings = b.connectedBuildings;
        this.connectedValues = b.connectedValues;
        this.totalWallArea = b.totalWallArea;
        this.lwater = b.lwater;
        this.lwTime = b.lwTime;
        this.wasEverWatered = b.wasEverWatered;
        this.inflameable = b.inflameable;
        this.totalRadiation = b.totalRadiation;
        this.radiationImpact = b.radiationImpact;
    ...
    public Building(int id) {
        ...
        burnRate = new GaussianGenerator(
                new Double(Configuration.getValue("resq-fire.burn-rate-average")).doubleValue(),
                new Double(Configuration.getValue("resq-fire.burn-rate-variance")).doubleValue(), random);
    }
    ...
    public void setTotalRadiation(double t) {
        this.totalRadiation = t;
    }

    public double getRadiationImpact() {
        return this.radiationImpact;
    }

    public void setRadiationImpact(double rad) {
        this.radiationImpact = rad;
    }
    ...
    public void reset(World w, int episodeNum) {
        // java.util.Random random = new java.util.Random(new
        // Long(Configuration.getValue("random.seed")).longValue()+ episodeNum);
        java.util.Random random = new java.util.Random(new Long(Configuration.getValue("random.seed")).longValue());
        burnRate = new GaussianGenerator( new Double(Configuration.getValue("resq-fire.burn-rate-average")).doubleValue(), new Double(Configuration.getValue("resq-fire.burn-rate-variance")).doubleValue(),random);
        setFieryness(0);
        setWaterQuantity(0);
        setIgnition(0);
        initialize(w);
        ...
        for (int x = 0; x < w.getAirTemp().length; x++)
            for (int y = 0; y < w.getAirTemp()[0].length; y++) {
                ...
                if (Geometry.boundingTest(polygon, xv, yv, w.SAMPLE_SIZE, w.SAMPLE_SIZE)) {
                    int pc = Geometry.percent((float) xv, (float) yv, (float) w.SAMPLE_SIZE, (float) w.SAMPLE_SIZE,
                            polygon);
                    if (pc > 0) {
            ...
            for (int x = 0; x < w.getAirTemp().length; x++) {
                for (int y = 0; y < w.getAirTemp()[0].length; y++) {
                    ...
                    if (Geometry.boundingTest(polygon, xv, yv, w.SAMPLE_SIZE, w.SAMPLE_SIZE)) {
                        ...
                        LOG.warn("pc = " + Geometry.percent((float) xv, (float) yv, (float) w.SAMPLE_SIZE,
                                (float) w.SAMPLE_SIZE, polygon));
                        int counter = 0;
